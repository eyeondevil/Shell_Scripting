...
#
#     Allow 90 seconds to answer before time-out
     read -t 90 answer
done
...
user_account=$answer
#
# Double check with the script whether this is the correct User Account to be deleted.
#
line1="Is $user_account is the user account to be removed: "
...
#     if user answers anything but "yes", exit script
#
exit_line1="Because the account, $user_account, does not exist: "
...
#
echo
echo "The user account record found: "
...
#
echo
echo "Step #2 - Find process on system belonging to this user account: "
echo
#
...
0)   # Processes running for this User Account.
     ...
     ps -u $user_account
     #
     line1="Would you like the script to kill these process(es)? [y/n]: "
     get_answer
     #
     answer=$(echo $answer | cut -c1)
     #
     ...
     y|Y)   # If user answers "yes",
            ...
            #
            echo
            echo "Killing all the process(es)..."
            #
            ...
            #
            echo
            echo "All the process(es) killed."
     ;;
...
echo
echo "Creating a report of all files owned by the $user_account."
echo
...
echo
echo "The user account, $user_account, has been removed successfully: "
echo
#
exit
